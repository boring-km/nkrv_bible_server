...
const searchOne = (label, chapter, paragraph) => new Promise((resolve) => {
    db((connection) => {
        const query = `select long_label, chapter, paragraph, sentence from bible2 where long_label = '${label}' and chapter = ${chapter} and paragraph = ${paragraph} and paragraph > 0;`;
        console.log(query);
...
const searchMultiLines = (label, chapter, start, end) => new Promise((resolve) => {
    db((connection) => {
        const query = `select long_label, chapter, paragraph, sentence from bible2 where long_label = '${label}' and chapter = ${chapter} and paragraph between ${start} and ${end} and paragraph > 0;`;
        console.log(query);
        connection.query(query, (err, results) => {
            if (err) {
...
const searchChapter = (label, chapter) => new Promise((resolve) => {
    db((connection) => {
        const query = `select long_label, chapter, paragraph, sentence from bible2 where long_label = '${label}' and chapter = ${chapter} and paragraph > 0;`;
        console.log(query);
        connection.query(query, (err, results) => {
            if (err) {
